...
**Language/Version**: Python 3.8+ (hosts; distribution-provided), ES2020 JavaScript (page)
...
| IV Least privilege | Per-host roles where they exist; shared-role exception documented | PASS with documented exception (curator shares the production role) |
...
2. Plant Reactome host: same list; it is a separate instance (`plant-reactome-host`), `<login>`; confirm systemd, `useradd` flags, ACL support, `setfacl` availability, AWS CLI path.
3. CPWS: where it runs and whether it meets the assumptions; decide in or out.
4. Which hosts share `EC2CloudwatchAgentRole` (known: hosts sharing the production role) and which have their own instance profile.
...
- Install script: `require_cmd systemctl python3 aws setfacl`, Python version check, instance
  identity check (`aws sts get-caller-identity` succeeds), and a hint for Amazon Linux package
  names when something is missing.
- CloudFormation: a `PerHostUploadPolicy` template section that creates a managed policy per
  host name given in a `HostRoles` parameter mapping host → role name (comma-separated list);
  the existing shared policy remains for the shared role.
