...
- [ ] T014 Scope each host's upload permission to its own `data/<host>/*` and `raw/<host>/*` per Constitution IV (partial) — HIGH. **Accepted exception (Constitution IV justification)**: the production host's instance role is shared with other machines, so its policy grants put and scoped list under all of `data/*` and `raw/*`; splitting the role means changing instance profiles on running production machines, a separate change. Mitigation: uploads are served as inert JSON (Constitution V). Hosts with their own role get per-host policies via `specs/001-multi-host-monitoring` T020. This task stays open as the record of the exception; do not implement it under 000. Tracked as issue #10.
